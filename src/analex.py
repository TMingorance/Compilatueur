...
class LexicalUnit(object):
    ...
    def __str__(self):
        unitValue = {'classname': self.__class__.__name__, 'lIdx': self.line_index, 'cIdx': self.col_index,
                     'length': self.length, 'value': self.value}
...
class LexicalAnalyser(object):
    ...
    def acceptKeyword(self, keyword):
        ...
        if self.lexical_units[self.lexical_unit_index].is_keyword(keyword):
            ...
            raise AnaLexException("Expecting keyword " + keyword + " <line " + str(
                self.lexical_units[self.lexical_unit_index].get_line_index()) + ", column " + str(
                self.lexical_units[self.lexical_unit_index].get_col_index()) + "> !")
    ...
    def acceptIdentifier(self):
        ...
        if self.lexical_units[self.lexical_unit_index].is_identifier():
            ...
            raise AnaLexException("Expecting identifier <line " + str(
                self.lexical_units[self.lexical_unit_index].get_line_index()) + ", column " + str(
                self.lexical_units[self.lexical_unit_index].get_col_index()) + "> !")
    ...
    def acceptInteger(self):
        ...
        if self.lexical_units[self.lexical_unit_index].is_integer():
            ...
            raise AnaLexException("Expecting integer <line " + str(
                self.lexical_units[self.lexical_unit_index].get_line_index()) + ", column " + str(
                self.lexical_units[self.lexical_unit_index].get_col_index()) + "> !")
    ...
    def acceptFel(self):
        ...
        if self.lexical_units[self.lexical_unit_index].is_fel():
            ...
            raise AnaLexException("Expecting end of program <line " + str(
                self.lexical_units[self.lexical_unit_index].get_line_index()) + ", column " + str(
                self.lexical_units[self.lexical_unit_index].get_col_index()) + "> !")
    ...
    def acceptCharacter(self, c):
        ...
        if self.lexical_units[self.lexical_unit_index].is_character(c):
            ...
            raise AnaLexException("Expecting character " + c + " <line " + str(
                self.lexical_units[self.lexical_unit_index].get_line_index()) + ", column " + str(
                self.lexical_units[self.lexical_unit_index].get_col_index()) + "> !")
    ...
    def acceptSymbol(self, s):
        ...
        if self.lexical_units[self.lexical_unit_index].is_symbol(s):
            ...
            raise AnaLexException("Expecting symbol " + s + " <line " + str(
                self.lexical_units[self.lexical_unit_index].get_line_index()) + ", column " + str(
                self.lexical_units[self.lexical_unit_index].get_col_index()) + "> !")
    ...
    ##Traceback of the current line
    def traceback(self):
        unit = self.lexical_units.pop(self.lexical_unit_index)
        #self.lexical_units.append(unit)
        errorline = ["line", str(unit.get_line_index()+1), "column", str(unit.get_col_index()+1), " --> "]
        self.lexical_units.insert(self.lexical_unit_index,unit)
        l = unit.get_line_index()
        for temp in self.lexical_units:
            if temp.get_line_index() == l:
                errorline.append(str(temp.get_value()))

        return str(' '.join(errorline))
